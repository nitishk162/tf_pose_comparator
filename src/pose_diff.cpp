...
void callback(const geometry_msgs::PoseStampedConstPtr &gt_pose, const geometry_msgs::PoseWithCovarianceStampedConstPtr &amcl)
{
	...
	//file recording
#if LOG_RECORD
	error_file<<amcl->pose.pose.position.x<<","<<amcl->pose.pose.position.y<<","<<errAmcl.x<<","<<errAmcl.y<<","<<errAmcl.theta<<","<<linearError<<","<<angularError<<endl;
#endif
...
#if LOG_RECORD
	error_file.open(path.c_str());
	error_file<<"TIME OF RUN"<<ros::Time::now()<<endl;
	error_file<<"amcl->pose.x"<<","<<"amcl->pose.y"<<","<<"errAmcl.x"<<","<<"errAmcl.y"<<","<<"errAmcl.theta"<<","<<"linearError"<<","<<"angularError"<<endl;
